...
public final class PlayerOverviewScreen extends BaseTabbedScreen {
    private static String lastSelectedTab = PlayerOverviewFormatter.TAB_SUMMARY;
    ...
    public void handleServerResponse(UiResponsePayload payload) {
        ...
        if (!payload.success()) {
            setError(payload.error() == null || payload.error().isBlank()
                    ? "Could not load player overview."
                    : payload.error());
            updateActionButtons();
            return;
        }
        ...
        try {
            ...
            setError("Could not read player overview data.");
        }
        updateActionButtons();
    }
    ...
    private void requestOverview(boolean forceRefresh) {
        loading = true;
        setLoading(true, Component.literal("Loading player overview..."));
        setError(null);
    ...
    private void requestWallet(boolean forceRefresh) {
        loading = true;
        setLoading(true, Component.literal("Loading wallet..."));
        setError(null);
    ...
    private void requestActivity(boolean forceRefresh) {
        loading = true;
        setLoading(true, Component.literal("Loading recent activity..."));
        setError(null);
    ...
    private void requestSummary(boolean forceRefresh) {
        loading = true;
        setLoading(true, Component.literal("Loading summary..."));
        setError(null);
    ...
    private Component listPanelTitle() {
        return switch (activeTabId()) {
            case PlayerOverviewFormatter.TAB_WALLET -> Component.literal("Wallet");
            case PlayerOverviewFormatter.TAB_ACTIVITY -> Component.literal("Recent Actions");
            case PlayerOverviewFormatter.TAB_SUMMARY -> Component.literal("Summary Items");
