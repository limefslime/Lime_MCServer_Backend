...
public final class ClientUiResponseDispatcher {
    ...
    private static void handleStatus(UiResponsePayload payload, Minecraft minecraft) {
        ...
        if (shouldAutoOpenActionScreen(payload)) {
    ...
    private static void handlePlayer(UiResponsePayload payload, Minecraft minecraft) {
        if (!isPlayerAction(payload.action())) {
            return;
        }
    ...
    private static boolean shouldAutoOpenActionScreen(UiResponsePayload payload) {
        return payload.action() != UiAction.OPEN && payload.success() && hasJsonData(payload);
    }

    private static boolean shouldAutoOpenJsonScreen(UiResponsePayload payload) {
        return shouldAutoOpenActionScreen(payload);
    }
    ...
    private static boolean isStatusAction(UiAction action) {
        return action == UiAction.OPEN
                || action == UiAction.STATUS_OVERVIEW
                || action == UiAction.STATUS_REFRESH;
    }

    private static boolean isPlayerAction(UiAction action) {
        return action == UiAction.OPEN
                || action == UiAction.PLAYER_OVERVIEW
                || action == UiAction.PLAYER_WALLET
                || action == UiAction.PLAYER_ACTIVITY
                || action == UiAction.PLAYER_SUMMARY
                || action == UiAction.PLAYER_REFRESH;
    }
}
